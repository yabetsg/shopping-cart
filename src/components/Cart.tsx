...
export const Cart = ({ children, totalPrice }: any) => {
  return (
    <div>
      <header className="p-10 text-4xl underline underline-offset-8">
        Your Shopping Cart
      </header>
      ...
      <div className="flex justify-around p-2 text-3xl border-t border-gray-300">
        <Link to={"/products"}>
          <button className="bg-[rgb(60,61,59)] rounded-sm text-white w-56 p-2">
            Back
          </button>
        </Link>
        ...
        {children ? (
          <button className="w-56 p-2 text-white bg-black rounded-sm">
            Checkout
          </button>
        ) : (
          "Cart is Empty"
